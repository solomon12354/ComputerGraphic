...
void addMidpointLineCellsColored(int x0, int y0, int x1, int y1) {
    ...
    while (1) {
        ...
        if (!(x0 == x1 && y0 == y1) && !isFilled(x0, y0)) {
            ...
            if (dxStep > 0 && dyStep > 0) { color = 3;  //printf("1"); 
            }      // 右上 → 藍色
            else if (dxStep > 0 && dyStep < 0) { color = 5; //printf("8");
            } // 右下 → 紫色
            else if (dxStep < 0 && dyStep < 0) { color = 6; //printf("6");
            } // 左下 → 白色
            else if (dxStep < 0 && dyStep > 0) { color = 4; //printf("3");
            }// 左上 → 黃色
            else color = 2;                                // 其他 → 綠色
