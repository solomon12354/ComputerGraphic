...
int triangleFillIndex = 0;
Cell triangleToFill[MAX_CELLS];  // 要填的格子座標

void collectTriangleCells(Cell a, Cell b, Cell c) {
    triangleFillIndex = 0;
    for (int x = 0; x < gridSize; ++x) {
        for (int y = 0; y < gridSize; ++y) {
            if (isPointInTriangle(x, y, a, b, c)) {
                triangleToFill[triangleFillIndex].x = x;
                triangleToFill[triangleFillIndex].y = y;
                triangleFillIndex++;
            }
        }
    }
...
int fillProgress = 0;
void fillTriangleAnimation(int value) {
    if (fillProgress < triangleFillIndex) {
        Cell c = triangleToFill[fillProgress];
        addColoredCell(c.x, c.y, 3);  // 綠色
        fillProgress++;
        glutPostRedisplay();
        glutTimerFunc(30, fillTriangleAnimation, 0);  // 下一格
    }
}
    ...
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
        ...
        if (fx >= -half && fx <= half && fy >= -half && fy <= half) {
            ...
            if (clickedCount >= 3) {
                ...
                collectTriangleCells(a, b, c);
                fillProgress = 0;
                glutTimerFunc(30, fillTriangleAnimation, 0);
