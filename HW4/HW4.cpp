...
void rotateAroundVector(float* x, float* y, float* z, float angle) {
    float dx = v2[0] - v1[0];
    float dy = v2[1] - v1[1];
    float dz = v2[2] - v1[2];
...
void myglRotatef(GLfloat angle, GLfloat x, GLfloat y, GLfloat z) {
    ...
    // 建立旋轉矩陣 (Rodrigues 旋轉公式)
    GLfloat rotationMatrix[16] = {
        cosA + x * x * oneMinusCos,      x * y * oneMinusCos - z * sinA,  x * z * oneMinusCos + y * sinA,  0,
        y * x * oneMinusCos + z * sinA,    cosA + y * y * oneMinusCos,    y * z * oneMinusCos - x * sinA,  0,
        z * x * oneMinusCos - y * sinA,    z * y * oneMinusCos + x * sinA,  cosA + z * z * oneMinusCos,    0,
        0,                           0,                         0,                         1
    };
...
void keyboard(unsigned char key, int x, int y) {
    ...
    case 'r':  // 重置
        ...
        break;
    case 't':  // 按t鍵讓方塊旋轉
        rotateAroundVector(&posX, &posY, &posZ, 5.0);
        isT = true;
        break;
    case 'g':  // 按t鍵讓方塊旋轉
        angleX -= 5.0f;
